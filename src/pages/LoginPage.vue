<template>
    <q-page class="flex flex-center" style="min-height:100vh;">
        <q-card class="q-pa-xl column items-stretch justify-around login-card">
            <div>

                <q-icon class="center" size="100px" :name="logo" />
                <div class="text-h4 text-left">Welcome back!</div>
            </div>


            <div class="colunm q-gutter-lg">
                <q-input v-model="email" color="secondary" filled label="Input your email" type="email" dense>
                    <template v-slot:before>
                        <q-icon name="mail" />
                    </template>
                </q-input>
                <q-input v-model="password" color="secondary" filled label="Input your password"
                    :type="isPwd ? 'password' : 'text'" dense>
                    <template v-slot:before>
                        <q-icon name="lock" />
                    </template>
                    <template v-slot:append>
                        <q-icon :name="isPwd ? 'visibility_off' : 'visibility'" class="cursor-poiner"
                            @click="isPwd = !isPwd" />
                    </template>
                </q-input>
            ...
            <div class="column q-gutter-md">
                <q-btn label="Log In" color="secondary" unelevated class="q-mt-md" />
                <q-btn label="Sing Up" color="secondary" outline class="q-mt-md" @click="goToRegister" />
            </div>
        </q-card>
