<template>
    <q-menu ref="menuRef" v-model="showMenu" no-focus :target="target" anchor="bottom left" self="top left"
        :offset="offset" transition-show="jump-down" transition-hide="jump-up">
        <q-list v-if="suggestions.length" style="min-width: 250px; max-height: 300px" class="scroll">
            <q-item v-for="(suggestion, index) in suggestions" :key="`${suggestion.type}-${suggestion.value}`" clickable
                @click="handleSelect(suggestion)" :active="index === selectedIndex" class="suggestion-item">
                <!-- Command suggestion -->
                <q-item-section v-if="suggestion.type === 'command'">
                    ...
                    <q-item-label caption class="text-grey-6 text-italic">{{ suggestion.usage }}</q-item-label>
                </q-item-section>
                ...
                <q-item-section v-if="suggestion.type === 'mention'">
                    ...
                    <q-item-label caption
                        :class="(suggestion as MentionSuggestion).user.isOnline ? 'text-positive' : 'text-grey-6'">
...
import type { Suggestion, SuggestionHandler, MentionSuggestion, CommandContext } from 'src/types/suggestions';
